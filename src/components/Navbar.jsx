    ...
    <div className="rounded-div flex items-center justify-between  h-20 font-bold">
      ...
      {user?.email ? (
        ...
        <div className="md:block hidden">
          ...
          <Link to='/signup' className='bg-button text-bntText px-5 py-2 rounded-2xl shadow-lg hover:shadow-2xl '>Sign Up</Link>
        </div>
      )}
      ...
      <div className={nav ? 'md:hidden fixed left-0 top-20 flex flex-col items-center justify-between w-full h-[90%] bg-primary ease-in duration-300 z-10'
        ...
        <div className='flex flex-col w-full p-4'>
          ...
          <Link to='/signup'>
            <button onClick={handleNav}  className='w-full my-2 p-3 bg-button text-bntText rounded-2xl shadow-xl'>Sign Up</button>
          </Link>
        </div>
      </div>
    </div>
